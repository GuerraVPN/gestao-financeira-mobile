...
interface TransactionFormProps {
  ...
  submitLabel?: string;
  onSubmitted?: () => void;
  transactionToEdit?: FinanceTransaction;
}
...
export function TransactionForm({
  ...
  const { state, addTransaction, editTransaction } = useFinance();
  ...
  const availableCategories = useMemo(
    () =>
      state.categories.filter((category) =>
        type === "transfer" ? true : category.type === type
      ),
  ...
  const resolvedCategoryId =
    type === "transfer"
      ? state.categories.find((category) => category.id === "cat-transporte")?.id ??
    ...
    <View className="gap-4 rounded-[28px] border border-border bg-surface p-5">
      <View className="gap-1">
        ...
        <Text className="text-sm text-muted">
          Registre movimentações rapidamente sem sair da tela atual.
        </Text>
      </View>
