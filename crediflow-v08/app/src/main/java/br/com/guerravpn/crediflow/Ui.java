...
final class Ui {
    ...
    void logo(LinearLayout r){LinearLayout row=new LinearLayout(a);row.setGravity(Gravity.CENTER_VERTICAL);TextView c=text("C",34,WHITE,true);c.setGravity(Gravity.CENTER);c.setBackground(bg(BLUE,18));row.addView(c,new LinearLayout.LayoutParams(dp(76),dp(76)));LinearLayout words=new LinearLayout(a);words.setOrientation(LinearLayout.VERTICAL);words.setPadding(dp(16),0,0,0);words.addView(text("CrediFlow",31,WHITE,true));words.addView(text("Crédito que impulsiona você",14,MUTED,false));row.addView(words,new LinearLayout.LayoutParams(0,-2,1));r.addView(row);}
